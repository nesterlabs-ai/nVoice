...
class VoiceAssistant:
    ...
    async def create_pipeline(self, transport: BaseTransport) -> Pipeline:
        ...
        # Input -> [NoiseHandler] -> STT -> [PreFilter] -> [InterruptionFilter] -> ToneAware -> Context -> LLM -> TextFilter -> TTS -> Output
        pipeline_processors = [
            transport.input(),
        ]

        # Add NoiseHandler after input to filter VAD false positives (rapid start/stop, short segments)
        if self.noise_handler:
            pipeline_processors.append(self.noise_handler)
            logger.info("🔇 NoiseHandler added to pipeline (after transport input)")
        ...
        # Add InterruptionFilter after PreFilter to validate interruptions with transcriptions
        # Allows barge-in when bot is speaking, holds/discards phantom interruptions when silent
        if self.interruption_filter:
            pipeline_processors.append(self.interruption_filter)
            logger.info("🛡️ InterruptionFilter added to pipeline (after PreFilter)")
    ...
    def create_task(self, enable_metrics: bool = True) -> PipelineTask:
        ...
        # allow_interruptions=True enables barge-in: when user speaks during bot output,
        # pipecat sends InterruptionFrame to stop TTS immediately.
        # Noise protection is handled by strict VAD settings (confidence=0.7, min_volume=0.5)
        # and the InterruptionFilterProcessor in the pipeline.
