...
async def get_graph_data(label: str = "*", max_depth: int = 3) -> Dict[str, Any]:
    """Proxy endpoint to fetch knowledge graph data from LightRAG.

    This endpoint proxies requests to LightRAG with proper authentication,
    so the API key stays on the server side.

    Query params:
        label: Node label to filter by (use '*' for all nodes)
    ...
    lightrag_url = os.getenv("LIGHTRAG_BASE_URL", "https://lightrag.nesterlabs.com").rstrip("/")
    api_key = os.getenv("LIGHTRAG_API_KEY", "")

    try:
        url = f"{lightrag_url}/graphs?label={label}&max_depth={max_depth}"
        headers = {
        ...
        logger.info(f"[Graph] Fetched {len(data.get('nodes', []))} nodes, {len(data.get('edges', []))} edges")
        return data

    except httpx.HTTPStatusError as e:
        logger.error(f"[Graph] LightRAG error: {e.response.status_code}")
        raise HTTPException(status_code=e.response.status_code, detail=f"LightRAG error: {e.response.status_code}")
    except Exception as e:
        logger.error(f"[Graph] Error fetching graph: {e}")
...
async def graph_health() -> Dict[str, Any]:
    ...
    try:
        ...
        async with httpx.AsyncClient(timeout=5.0) as client:
            response = await client.get(f"{lightrag_url}/health", headers=headers)
            response.raise_for_status()
            return {"status": "healthy", "lightrag": response.json()}
    except Exception as e:
